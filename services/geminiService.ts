...
import type { Language, Domain, OutputLength, PromptType, ApiKeyStatus, ApiKeyStorage, ApiKeyState } from '../types';
import { 
  translations, 
  GEMINI_MODEL_NAME, 
  API_KEY_STORAGE_KEY, 
...
export const saveApiKey = (apiKey: string, storage: ApiKeyStorage): void => {
  if (storage === 'none' || !apiKey) {
    return;
  }
  
  const storageObj = storage === 'session' ? sessionStorage : localStorage;
  const apiKeyData: ApiKeyState = {
    key: apiKey,
    status: 'valid',
    storage,
...
export const getStoredApiKey = (): ApiKeyState | null => {
  ...
  try {
    const apiKeyData: ApiKeyState = JSON.parse(storedData);
...
export const generateStructuredPromptWithGemini = async (
  params: {
    rawRequest: string;
...
const constructMetaPrompt = (params: {
  ...
  mission: string;
  constraints: string;
}): { systemInstruction: string; userQuery: string } => {
  const {
    rawRequest,
    promptType,
    domain,
